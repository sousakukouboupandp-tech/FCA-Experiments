# -*- coding: utf-8 -*-
"""solve_D0.py のロジックを縮小世界に当てて、参照 solver と一致するか確認する。
★実寸を回す前の最後の関門。C を開く前に実装の正しさを確かめる★
solve_D0 と同じ式（t=999 の扱い、方策の決め方、optimal_set のビット）を使う。
"""
...
def solve_D0_like(horizon):
    """★solve_D0.py と同じ式で縮小世界を解く★"""
    ...
    t_owner = np.array(t_owner, dtype=np.int64); t_prob = np.array(t_prob, dtype=np.float64)
...
def main(which="A"):
    if which == "B":
        m.HORIZON = 40
    H = m.HORIZON
    print(f"solve_D0 のロジックを縮小{which}（H={H}）で検証する")
    print()
    ...
    print()
    print(f"初期状態の V: 参照 {value(m.DECISION, *init, 0):.6f}"
          f" / solve_D0式 {Vs[0][i0]:.6f}")

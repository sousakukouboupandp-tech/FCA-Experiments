# -*- coding: utf-8 -*-
"""
共鳴実験 v03 本実行（15シード×4条件×3世界）
【v02からの変更】η_res=0.5(書記官が置いた暫定係数)を廃止し1.0に。
 受け取りの門番をシンクロ率Sだけに一本化する（思想への忠実化）。
 v02の統計判定: FullCopyの守りはp<0.01で有意、ResoExpの守りはn.s.だった。
 原因仮説: η×Sの二重値引き。v03でこの仮説を検証。まだn.s.ならL1を認定し失敗として公開する。
【v01からの変更＝身に染みる項（設計書§15・著者の思想注入第2弾）】
...
ETA_RES = 1.0  # v03: 門番はシンクロ率Sのみ。暫定係数の二重値引きを廃止
LAMBDA_OWN = 0.97
...
if __name__ == "__main__":
    ...
    json.dump(all_out, open(os.path.join(BASE, 'resonance_v03_full_results_15seed.json'),
